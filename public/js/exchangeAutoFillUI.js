...
export class ExchangeAutoFillUI {
        ...
        <div class="modal-content" role="dialog" aria-labelledby="autoExchangeModalTitle">
          ...
          <div id="form-exchange-content">
            ...
            <div id="exchange-inputs">
              <label>
                <input type="radio" name="exchange-type" value="blue" checked /> Usar Dólar Blue
              </label>
              <label>
                <input type="radio" name="exchange-type" value="oficial" /> Usar Dólar Oficial
              </label>
              <hr />
              <div>
                <label>
                  <input type="radio" name="apply-scope" value="all" checked /> Aplicar a todos los meses
                </label>
                <label>
                  <input type="radio" name="apply-scope" value="empty" /> Aplicar solo a meses sin TC
                </label>
              </div>
            </div>
   ...
   * Se evita enviar solicitudes a la API para meses que sean posteriores al mes actual.
   * Además, se utiliza el método batchGet para realizar las solicitudes de forma paralela.
  ...
  async fillExchangeRates(selectedExchange, selectedScope) {
    ...
    // Preparar un array de solicitudes para los meses elegibles
    const requests = [];
    periodo.meses.forEach((mes, index) => {
      ...
      // Evitar solicitudes para meses futuros
      if (
        fullYear > currentYear ||
        (fullYear === currentYear && monthNumber > currentMonth)
      ) {
        console.log(`Skipping API request for future month: ${mes.mes}`);
        return;
      }
      const dayForAPI = "05"; // Día por defecto
      const fecha = `${fullYear}-${mm}-${dayForAPI}`;
      const endpoint = `/historical?day=${fecha}`;
      requests.push({ index, endpoint });
    });

    // Si no hay solicitudes a realizar, actualizamos la UI y salimos
    ...
    let responses;
    try {
      responses = await this.cambioDolarBlue.batchGet(endpoints);
    } catch (error) {
      throw new Error(`Error realizando batch GET: ${error.message}`);
    }

    // Procesar las respuestas y actualizar cada mes correspondiente
    requests.forEach((req, i) => {
      const data = responses[i];
      const mes = periodo.meses[req.index];
    ...
    // Actualizar el servicio y la UI
    this.uiManager.periodoService.actualizarPeriodo(periodo);
    this.uiManager.periodDetailUI.renderMeses(periodo);
    this.uiManager.calcularYMostrarMetricas();
    if (errores.length > 0) {
